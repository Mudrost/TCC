...
def simulate_motor(params, is_optimizing = False):
    ...
    timestep = 0.002                                                            # Passo de simulação
    mass_flow = [0]
    burn_rate = [knsb['r0']]
    # Loop de queima
    while (True):
...
burning_area, chamber_pressure, kn, pe, thrust_coef, thrust, burn_time, total_impulse, mass_flow, time, pressure, burn_rate = simulate_motor([0.024074329350030548,0.10499517455735301,0.011857890735258609])
...
f= plot.figure()
ax = f.gca()
plot.plot(time, list(map(lambda n: get_optimal_expansion_ratio(n,k =knsb['k'], ambient_pressure = ambient_pressure), chamber_pressure)))
ax.set_ylim(ymin=0)
plot.xlabel("Tempo (s)")
plot.ylabel("Ae/A*")
#plot.title("Pressure vs time")
